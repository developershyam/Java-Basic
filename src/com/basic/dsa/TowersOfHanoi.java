...
public class TowersOfHanoi {
	...
	public void doTOH(int n, String source, String auxiliary, String destination, boolean showInfo) {
		...
		if (showInfo) {
			showInfo = false;
			System.out.println("source: " + source  + ", auxiliary: " + auxiliary+", destination: " + destination);
		}
		
		if (n == 1) {
			System.out.println("Disc moved from  " + source + " -> " + destination);
		} else {
			doTOH(n - 1, source, destination, auxiliary, showInfo);
			
			doTOH(1, source, auxiliary, destination, showInfo);
